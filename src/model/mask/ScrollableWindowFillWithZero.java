...
public class ScrollableWindowFillWithZero implements ScrollableWindow {
	...
	public ScrollableWindowFillWithZero(ImageGray img, int widthWindow, int heightWindow) {
		this.img = img;
	...
	public double[][] nextRegion() {
		...
		for (Point displacement : this.displacements) {
			...
			if (x < 0 || x >= img.getWidth() || y < 0 || y >= img.getHeight()) {
				region[this.middleWindow.x + displacement.x][this.middleWindow.y + displacement.y] = 0;
			} else {
